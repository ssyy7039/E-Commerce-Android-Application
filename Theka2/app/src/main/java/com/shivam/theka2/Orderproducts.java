...
public class Orderproducts extends AppCompatActivity {
    ...
    String Key,Shopname,status1,Status="Order is cancelled from the Customer side",Status2="You cancelled the order",Time,Orderid,uid;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (status1.equals("Order is Cancelled from the Shopkeeper side")){
        ...
        databaseReference = FirebaseDatabase.getInstance().getReference("All Customers").child(uid).child("All Orders").child(Key).child("drinks");
    ...
    public void Dialogcancel(){
        ...
        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                //   checkConnection();
                final ProgressDialog progressDialog=new ProgressDialog(com.shivam.theka2.Orderproducts.this);
                ...
                FirebaseDatabase.getInstance().getReference("All Customers Orders")
                        .child(Shopname).child(Key).setValue(requests).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(com.shivam.theka2.Orderproducts.this, "Order is not cancelled", Toast.LENGTH_SHORT).show();
                    }
                });

//                final DatabaseReference databaseReference2= FirebaseDatabase.getInstance().getReference("All Customers").child(uid).child("All Orders");
//                databaseReference2.child(Key).removeValue();
                ...
                FirebaseDatabase.getInstance().getReference("All Customers")
                        .child(uid).child("All Orders").child(Key).setValue(requests2).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()){
                            finish();
                    ...
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(com.shivam.theka2.Orderproducts.this, "your order is not cancelled", Toast.LENGTH_SHORT).show();
                    }
                });
                progressDialog.dismiss();
                Toast.makeText(com.shivam.theka2.Orderproducts.this, "Your Order is cancelled", Toast.LENGTH_SHORT).show();
    ...
    public void Dialogdelete(){
        ...
        builder.setPositiveButton("YES", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                databaseReference2.child(Key).removeValue();
                progressDialog.dismiss();
                Toast.makeText(com.shivam.theka2.Orderproducts.this, "Your Order deleted", Toast.LENGTH_SHORT).show();
                //  startActivity(new Intent(getApplicationContext(),Allorders.class));
                finish();
